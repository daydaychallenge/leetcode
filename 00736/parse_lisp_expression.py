...
class Let:
    def __init__(self, var_expr_list, child_expr):
        self.var_expr_list = var_expr_list
        self.child_expr = child_expr

    def eval(self, var_map):
        new_map = dict(var_map)
        for v, e in self.var_expr_list:
            new_map[v.vid] = e.eval(new_map)
...
class LispParser:
    ...
    def parse_var_expr(self):
        var_list = []
        while self.lookahead() != '(' and (self.idx + 1 < len(self.tokens) and self.lookahead(1) != ')'):
